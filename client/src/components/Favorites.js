...
class Favorites extends Component {
  ...
  componentWillReceiveProps({ data, deleted, added }) {
    // make sure that only jobs which are starred are set to state
    const onFavorites = jobs => jobs.filter(job => job.favorite === true);
    if (data !== this.props.data) this.setState({ jobNotes: onFavorites(data.applications) });
    if (deleted !== this.props.deleted) this.setState({ jobNotes: onFavorites(deleted.applications) });
    if (added !== this.props.added) this.setState({ jobNotes: onFavorites(added.applications) });
  }
    ...
    return (
      <Container content={this.state.jobNotes && this.state.jobNotes.length}>
        ...
        }
        {
           ...
           />) :
            (<Instructions>{'Add your favorite job applications to the Favorites List to see em\' here.'}</Instructions>)
        }
        <AddWrapper>
          <Add
            name="plus-circle"
